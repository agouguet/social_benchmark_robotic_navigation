...
class HBSNAgentNode:
    ...
    def __init__(self, mdp, parent, state, qfunction, bandit, reward=0.0, action=None):
        ...
        self.parent = parent
        self.state = state
        self.id = HBSNAgentNode.next_node_id
        HBSNAgentNode.next_node_id += 1
    ...
    def back_propagate(self, reward, child):
        ...
        HBSNAgentNode.visits[self.state] = HBSNAgentNode.visits[self.state] + 1
        HBSNAgentNode.visits[(self.state, action)] = HBSNAgentNode.visits[(self.state, action)] + 1

        delta = (1 / (HBSNAgentNode.visits[(self.state, action)])) * (
    ...
    def get_visits(self):
        return HBSNAgentNode.visits[self.state]

    def reset_visits():
        HBSNAgentNode.next_node_id = 0
        HBSNAgentNode.visits = defaultdict(lambda: 0)


class HBSNAgentMCTS:
    def __init__(self, mdp, qfunction, bandit, heuristic_function):
        self.mdp = mdp
        self.qfunction = qfunction
        self.bandit = bandit
    ...
    def mcts(self, state, timeout=1, sleep=True):
        self.first=True
        root_node = HBSNAgentNode(self.mdp, None, state, self.qfunction, self.bandit)
    ...
    def choose(self, state, prev_actions):
        action_choosen = self._heuristic_function(self.mdp, state)
