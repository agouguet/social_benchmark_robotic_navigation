...
class SRNNPolicy(RecurrentActorCriticPolicy):
    def __init__(
        self,
        observation_space,
        action_space,
        ...
        **kwargs
        ...
        self.srnn_model = None
        self.config = config  # Pass your config from outside
        super().__init__(
            observation_space,
            action_space,
            lr_schedule,
    ...
    def _build_mlp_extractor(self):
        # No need to build a separate mlp, we use SRNN instead
        self.mlp_extractor = None

        # Initialize SRNN
        self.srnn_model = SRNN(self.observation_space, self.config)

        # Define action distribution
        self.action_dist = DiagGaussianDistribution(self.srnn_model.output_size)

    def forward(self, obs: torch.Tensor, lstm_states: RNNStates, episode_starts: torch.Tensor, deterministic: bool = False) -> Tuple[torch.Tensor, torch.Tensor, RNNStates]:
        masks = episode_starts.float().unsqueeze(-1)
        rnn_hxs = {"rnn": lstm_states[0]}  # Assuming lstm_states[0] is the hidden state

        value, latent_pi, rnn_hxs = self.srnn_model(obs, rnn_hxs, masks, infer=True)

        actions, log_prob = self.action_dist.sample(latent_pi, deterministic=deterministic)
        return actions, value, rnn_hxs["rnn"]

    def evaluate_actions(
        self, obs: torch.Tensor, lstm_states: RNNStates, episode_starts: torch.Tensor, actions: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, RNNStates]:
        masks = episode_starts.float().unsqueeze(-1)
        rnn_hxs = {"rnn": lstm_states[0]}

        value, latent_pi, rnn_hxs = self.srnn_model(obs, rnn_hxs, masks, infer=True)

        log_prob, entropy = self.action_dist.log_prob(actions, latent_pi), self.action_dist.entropy(latent_pi)
        return value, log_prob, entropy, rnn_hxs["rnn"]

    def get_distribution(self, obs: torch.Tensor, lstm_states: RNNStates, episode_starts: torch.Tensor):
        masks = episode_starts.float().unsqueeze(-1)
        rnn_hxs = {"rnn": lstm_states[0]}

        _, latent_pi, rnn_hxs = self.srnn_model(obs, rnn_hxs, masks, infer=True)
        return self.action_dist.proba_distribution(latent_pi), rnn_hxs["rnn"]

    def get_latent(self, obs: torch.Tensor, lstm_states: RNNStates, episode_starts: torch.Tensor):
        masks = episode_starts.float().unsqueeze(-1)
        rnn_hxs = {"rnn": lstm_states[0]}
        _, latent_pi, rnn_hxs = self.srnn_model(obs, rnn_hxs, masks, infer=True)
        return latent_pi, rnn_hxs["rnn"]
