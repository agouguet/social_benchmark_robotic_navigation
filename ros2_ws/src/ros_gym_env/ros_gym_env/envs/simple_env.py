...
class RosSimpleEnv(gym.Env):
    def __init__(self, env_id, config, env_id_display_log=None):
        ...
        self.observation_space = gym.spaces.Box(low=-math.inf, high=math.inf, shape=(self.scan_size + 3,), dtype=np.float32)
        ...
        self.node.create_subscription(CNNdata, self.prefix + self.config.env.ros.cnn_data, self._cnn_data_callback, 1)
    ...
    def _get_observation(self):
        ...
        # scan:
        self.scan_simple = np.clip(self.cnn_data.scan[-self.scan_size:], 0.0, 10.0)
        # goal:
    ...
    def _theta_reward(self, goal, mht_peds, v_x, r_angle, angle_thresh):
        ...
        if (self.env_id_display_log == self.env_id or self.env_id_display_log == None):
            ...
            # self.node.get_logger().warning("{} {} {} {} {}".format(goal, mht_peds, v_x, r_angle, angle_thresh))
        return reward  
